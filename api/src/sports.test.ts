  ...
  describe('#getWeekDescription()', () => {
    describe('NCAAB', () => {
      ...
      test('is Round 1 for Mar 19th', () => {
        expect(ncaab.getWeekDescription(
          { startTime: new Date(`${ncaab.seasonYear + 1}-03-19`) } as IGame,
        )).toEqual('Round 1');
      });

      test('is Round 2 for Mar 21st', () => {
        expect(ncaab.getWeekDescription(
          { startTime: new Date(`${ncaab.seasonYear + 1}-03-21`) } as IGame,
        )).toEqual('Round 2');
      });

      test('is Sweet 16 for Mar 26th', () => {
        expect(ncaab.getWeekDescription(
          { startTime: new Date(`${ncaab.seasonYear + 1}-03-26`) } as IGame,
        )).toEqual('Sweet 16');
      });

      test('is Elite 8 for Mar 28th', () => {
        expect(ncaab.getWeekDescription(
          { startTime: new Date(`${ncaab.seasonYear + 1}-03-28`) } as IGame,
        )).toEqual('Elite 8');
      });

      test('is Final Four for Apr 4th', () => {
        expect(ncaab.getWeekDescription(
          { startTime: new Date(`${ncaab.seasonYear + 1}-04-04`) } as IGame,
        )).toEqual('Final Four');
      });

      test('is Championship for Apr 6th', () => {
        expect(ncaab.getWeekDescription(
          { startTime: new Date(`${ncaab.seasonYear + 1}-04-06`) } as IGame,
    ...
    describe('XFL', () => {
      ...
      test('is Playoffs for Apr 18th', () => {
        expect(xfl.getWeekDescription(
          { startTime: new Date(`${xfl.seasonYear}-04-18`) } as IGame,
        )).toEqual('Playoffs');
      });

      test('is Championship for Apr 26th', () => {
        expect(xfl.getWeekDescription(
          { startTime: new Date(`${xfl.seasonYear}-04-26`) } as IGame,
        )).toEqual('Championship');
      });
    });
  });
