...
function generateTimeString(game) {
  if (game.q === 'F' || game.q === 'FO') { return 'Final'; } else if (game.q === 'H') { return 'Half'; }
  return `${game.q}Q ${game.k}`;
}

module.exports.parseXML = (scoresXML, cb) => {
  xml2js.parseString(scoresXML, (err, scoresJSON) => {
    ...
    return cb(null, { year, weekNum, scores });
  });
};
